...
interface RestoreWtfModalProps {
  restoreState: 'confirm' | 'loading'
  onConfirm: () => void
  onCancel: () => void
}

export default function RestoreWtfModal({
  restoreState,
  onConfirm,
  onCancel
}: RestoreWtfModalProps): React.JSX.Element {
        ...
        {restoreState === 'confirm' ? (
          <>
            <h2 className="font-display text-2xl font-semibold leading-8 text-cyan-100/90 max-w-sm">
              {t('components.modals.restoreWtf')}
            </h2>
            <div className="mt-8 flex items-center justify-center gap-12">
              ...
              <button
                type="button"
                onClick={onConfirm}
                className="hover-glow text-3xl font-bold text-yellow-500 transition-all hover:text-yellow-300 cursor-pointer outline-none focus:outline-none focus:ring-0"
              >
                {t('components.modals.restore')}
              </button>
            </div>
          </>
        ) : (
          <>
            ...
            <p className="text-gray-400">{t('components.modals.restoreLoading')}</p>
          </>
        )}
      </div>
    </div>
